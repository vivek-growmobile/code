...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0, self.width - 1) :
            for y in range(0, self.height - 1) :
                new_point = Point(x,y)
                curr = Block(new_point, color_rgb(random.randint(0,255), random.randint(0,255), random.randint(0,255)))
                self.block_list[new_point] = curr
    ...
    def get_block_neighbors(self, block):
        ...
        #### YOUR CODE HERE #####
        neighbors = []
        ref_point = Point(block.x, block.y)

        #Top left block
        if block.x - 1 >= 0 and block.y - 1 >= 0 :
            for square in self.block_list.values() :
                if square.x == block.x - 1 and square.y == block.y - 1 :
                    neighbors.append(square)
            
        #Top middle block
        if block.y - 1 >= 0 :
            for square in self.block_list.values() :
                if square.x == block.x and square.y == block.y - 1 :
                    neighbors.append(square)
        

        #Top right block
        if block.x + 1 <= 9 and block.y - 1 >= 0 :
            for square in self.block_list.values() :
                if square.x == block.x + 1 and square.y == block.y - 1 :
                    neighbors.append(square)
        ...
        if block.x - 1 >= 0 :
            for square in self.block_list.values() :
                if square.x == block.x - 1 and square.y == block.y :
                    neighbors.append(square)
        ...
        #Bottom left block
        if block.x - 1 >= 0 and block.y + 1 <= 9 :
            for square in self.block_list.values() :
                if square.x == block.x - 1 and square.y == block.y + 1 :
                    neighbors.append(square)
        ...
        if block.y + 1 <= 9 :
            for square in self.block_list.values() :
                if square.x == block.x and square.y == block.x + 1 :
                    neighbors.append(square)

        #Bottom right block
        if block.x + 1 <= 9 and block.y + 1 <= 9 :
            for square in self.block_list.values() :
                if square.x == block.x + 1 and square.y == block.y + 1 :
                    neighbors.append(square)
